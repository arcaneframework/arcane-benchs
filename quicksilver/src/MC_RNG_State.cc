...
void 
breakup_uint64(uint64_t uint64_in, uint32_t& front_bits,
                    uint32_t& back_bits)
{
...
void 
pseudo_des(uint32_t& lword, uint32_t& irword)
{
...
uint64_t
reconstruct_uint64(uint32_t front_bits, uint32_t back_bits)
{
...
uint64_t
hash_state(uint64_t initial_number)
{
...
Int64 
rngSpawn_Random_Number_Seed(Int64* parent_seed)
{
  Int64 spawned_seed = hash_state(*parent_seed);
